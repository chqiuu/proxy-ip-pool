...
public class AsyncConfig {
    ...
    public ThreadPoolTaskExecutor getValidateProxyAsyncExecutor() {
        ...
        // 线程池最大线程数,默认：40000
        taskExecutor.setMaxPoolSize(processors * 7);
        // 线程队列最大线程数,默认：80000
        taskExecutor.setQueueCapacity(processors * 10);
        // 线程名称前缀
    ...
    public ThreadPoolTaskExecutor getTimeoutAsyncExecutor() {
        // Java虚拟机可用的处理器数
        int processors = Runtime.getRuntime().availableProcessors();
        // 定义线程池
        ThreadPoolTaskExecutor taskExecutor = new ThreadPoolTaskExecutor();
        // 核心线程数
        taskExecutor.setCorePoolSize(processors * 6);
        // 线程池最大线程数,默认：40000
        taskExecutor.setMaxPoolSize(processors * 7);
        // 线程队列最大线程数,默认：80000
        taskExecutor.setQueueCapacity(processors * 10);
        // 线程名称前缀
        taskExecutor.setThreadNamePrefix("async-timeout");
        // 如果设置为 true，那么核心线程数也将受keepAliveTime控制
        taskExecutor.setAllowCoreThreadTimeOut(true);
        // 线程池中线程最大空闲时间，默认：60，单位：秒
        taskExecutor.setKeepAliveSeconds(60);
